...
const rodLimit=count=>{
 const ys=BASE[count],edge=30;
 return Math.max(0,Math.min(ys[0]-edge,(H-edge)-ys[ys.length-1]));
};
  ...
  for(let i=0;i<R.length;i++){
   ...
   for(let j=0;j<ys.length;j++){
    ...
    if(rr.kick>0&&rr.kick<=.50&&!rr.hit){
     const reach=this.boot(rr)+18,localX=(b.x-r.x)*d,localY=b.y-(py+4);
 ...
 step(dt){
  ...
  for(const team of ["green","blue"]){
   ...
   for(let i=0;i<R.length;i++)if(R[i].team===team){
    const rr=this.s.rods[i],old=rr.offset,limit=rodLimit(R[i].count);
    rr.offset=clamp(old+mv,-limit,limit);
  ...
  if(speed<1.2){b.vx=0;b.vy=0}
  else{
   const decel=20+(speed*speed)*.00018,ns=Math.max(0,speed-decel*dt),f=ns/speed;
   b.vx*=f;b.vy*=f;
  }
